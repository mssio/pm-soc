...
# Wi-Fi heuristics
WIFI_CLOSE_DBM = float(os.getenv("SOCPI_WIFI_CLOSE_DBM", "-40"))  # "very close" AP threshold
...
<head>
  ...
  <style>
    ...
    *{ box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
    ...
    /* Toast: pop-up notifications for new device + changes */
    #toast{
      position:fixed; left:50%; transform:translateX(-50%);
      bottom: 86px; /* above tabs on 7" touchscreen */
    ...
    #toast.info{ border-color: rgba(0,242,255,.45); }
    #toast.medium{ border-color: rgba(255,171,0,.65); }
    #toast.critical{ border-color: rgba(255,77,77,.75); }
...
// --- POPUP / TOAST (used for security events too) ---
const seenEvents = new Set();
let eventsInitialized = false;

function toast(msg, sev="INFO"){
  const t = document.getElementById("toast");
  t.classList.remove("info","medium","critical");
  const s = (sev || "INFO").toUpperCase();
  if(s === "CRITICAL") t.classList.add("critical");
  else if(s === "MEDIUM") t.classList.add("medium");
  else t.classList.add("info");

  t.textContent = msg;
  t.style.display = "block";
  clearTimeout(window.__toastTimer);
  const dur = (s === "CRITICAL") ? 4200 : (s === "MEDIUM" ? 3000 : 2200);
  window.__toastTimer = setTimeout(() => { t.style.display = "none"; }, dur);
}

function eventKey(e){
  return `${e.ts||""}|${e.kind||""}|${e.title||""}`;
}

async function pollAlerts(){
  try{
    const j = await fetch("/api/alerts?limit=30").then(r => r.json());
    const evts = j.events || [];

    // first run: mark existing events as seen (no spam on boot)
    if(!eventsInitialized){
      evts.forEach(e => seenEvents.add(eventKey(e)));
      eventsInitialized = true;
      return;
    }

    // show only new events, oldest-first
    const newOnes = evts.filter(e => !seenEvents.has(eventKey(e)));
    newOnes.reverse().forEach(e => {
      seenEvents.add(eventKey(e));
      const sev = e.severity || "INFO";
      const kind = e.kind || "EVENT";
      const title = e.title || "";
      toast(`${kind}: ${title}`, sev);
    });
  }catch(e){
    // ignore
  }
...
function pill(text){
  return `<span class="pill" style="display:inline-flex; min-height:auto; padding:4px 8px; font-size:.75rem;">${text}</span>`;
}
...
function renderRadio(r){
  ...
  wifiList.innerHTML = aps.length
    ? aps.slice(0,10).map(a => {
        const flags = (a.flags || []).map(f => pill(f)).join(" ");
        const ssid = (a.ssid && a.ssid.length) ? a.ssid : "<hidden>";
        return `<div style="font-family:JetBrains Mono; font-size:.9rem; margin-bottom:10px; line-height:1.25;">
          ${ssid}
          ${pill(a.bssid)}
          ${a.signal ? pill(a.signal) : ""}
          ${flags ? " " + flags : ""}
        </div>`;
      }).join("")
  ...
  btList.innerHTML = devs.length
    ? devs.slice(0,12).map(d => {
        const flags = (d.flags || []).map(f => pill(f)).join(" ");
        return `<div style="font-family:JetBrains Mono; font-size:.9rem; margin-bottom:10px; line-height:1.25;">
          ${d.name || "Unknown"} ${pill(d.mac)} ${flags ? " " + flags : ""}
        </div>`;
      }).join("")
...
// NEW: poll alerts and pop toast notifications for NEW DEVICE / CHANGES
...
def parse_dbm(signal_str: str) -> Optional[float]:
    """
    Accepts strings like "-47.00" or "-47.00 dBm" or "-47 dBm"
    Returns float or None
    """
    if not signal_str:
        return None
    m = re.search(r"(-?\d+(?:\.\d+)?)", signal_str)
    if not m:
        return None
    try:
...
# ----------------------
# RADIO SENSORS (Wi-Fi / BT) + HEURISTICS
# ----------------------
def wifi_scan_iw(iface: str) -> List[dict]:
    ...
    for line in out.splitlines():
        ...
        if line.startswith("BSS "):
            if bssid:
                dbm = parse_dbm(signal_str or "")
                flags = []
                if not (ssid or "").strip():
                    flags.append("HIDDEN_SSID")
                if dbm is not None and dbm >= WIFI_CLOSE_DBM:
                    flags.append("CLOSE_AP")
                aps.append({"bssid": bssid, "ssid": ssid or "", "signal": (signal_str or ""), "signal_dbm": dbm, "flags": flags})
            ...
            # keep friendly format
            raw_dbm = parts.split()[0] if parts else ""
            signal_str = (raw_dbm + " dBm") if raw_dbm else ""
...
def bt_classify(name: str) -> Tuple[str, List[str]]:
    """
    Lightweight BLE/BT heuristic flags (no heavy probing).
    Returns (severity, flags)
    """
    n = (name or "").strip()
    nl = n.lower()
    flags: List[str] = []
    ...
    if not n or n.lower() in ("unknown", "n/a", "device"):
        flags.append("UNKNOWN_NAME")
        sev = "MEDIUM"

    # Common dev boards / IoT-ish markers
    iot_markers = ["esp32", "esp", "hc-05", "hc-06", "arduino", "ble", "sensor", "tracker", "tile", "mi band"]
    if any(m in nl for m in iot_markers):
        flags.append("IOT/DEV_BOARD")
        sev = "MEDIUM"

    # Audio devices (usually low risk, but still track)
    audio_markers = ["jbl", "beats", "airpods", "sony", "bose", "anker", "soundcore"]
    if any(m in nl for m in audio_markers):
        flags.append("AUDIO")

    # HID peripherals
    hid_markers = ["keyboard", "mouse", "logitech", "mx", "magic", "trackpad"]
    if any(m in nl for m in hid_markers):
        flags.append("HID")

    # "Setup"/pairing patterns can be abused (weak pairing)
    setup_markers = ["setup", "pair", "connect", "provision"]
    if any(m in nl for m in setup_markers):
        flags.append("PAIRING_MODE?")
        sev = "MEDIUM"
...
def bt_scan_bluetoothctl() -> List[dict]:
    ...
    for line in out.splitlines():
        ...
        if len(parts) >= 3:
            ...
            sev, flags = bt_classify(name)
            devs.append({"mac": mac, "name": name, "severity": sev, "flags": flags})
    return devs
    ...
    while True:
        ...
        try:
            if WIFI_ENABLED and t >= next_wifi:
                ...
                new_bssids = sorted(bssids - prev_wifi_bssids)

                # New APs event
                if new_bssids:
                    emit_event("WIFI_NEW_AP", "INFO", f"New Wi-Fi AP(s) detected: {len(new_bssids)}", {"bssids": new_bssids[:20]})

                # Close AP heuristic (strong signal)
                close = [a for a in aps if (a.get("signal_dbm") is not None and a["signal_dbm"] >= WIFI_CLOSE_DBM)]
                if close:
                    emit_event(
                        "WIFI_CLOSE_AP",
                        "MEDIUM",
                        f"Very close Wi-Fi AP(s): {len(close)} (strong signal)",
                        {"aps": [{"ssid": a.get("ssid",""), "bssid": a.get("bssid"), "signal": a.get("signal")} for a in close[:10]]},
                    )

                # Duplicate SSID heuristic (evil twin indicator)
                by_ssid: Dict[str, List[str]] = {}
                for a in aps:
                    s = (a.get("ssid") or "").strip()
                    if not s:
                        continue
                    by_ssid.setdefault(s, []).append(a.get("bssid", ""))
                dup = {s: bs for s, bs in by_ssid.items() if len(set(bs)) > 1}
                if dup:
                    # Keep it INFO: many legit networks have multiple APs; it's just a watch signal
                    sample = list(dup.items())[:5]
                    emit_event("WIFI_DUP_SSID", "INFO", f"Duplicate SSID(s) observed: {len(dup)}", {"samples": sample})
            ...
            if BT_ENABLED and t >= next_bt:
                ...
                new_macs = sorted(macs - prev_bt_macs)

                # New BT device event with severity heuristic
                if new_macs:
                    # If any new device is MEDIUM, raise event to MEDIUM
                    sev = "INFO"
                    new_dev_objs = [d for d in devs if d.get("mac") in set(new_macs)]
                    if any((d.get("severity") or "").upper() == "MEDIUM" for d in new_dev_objs):
                        sev = "MEDIUM"
                    emit_event("BT_NEW_DEVICE", sev, f"New Bluetooth device(s) detected: {len(new_macs)}", {"devices": new_dev_objs[:12]})
