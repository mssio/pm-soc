...
DEPENDENCIES:
  sudo apt-get update
  sudo apt-get install -y nmap wireless-tools iw bluez
  sudo pip3 install flask psutil requests

ENV VARS (optional):
  SOCPI_PORT=8080
  SOCPI_CIDR=192.168.0.0/24
  SOCPI_SCAN_INTERVAL=60
  SOCPI_PROFILE_TTL=300
  SOCPI_TOP_PORTS=50

  SOCPI_OFFLINE_AFTER=180          # seconds since last seen to mark offline
  SOCPI_WIFI_ENABLED=true
  SOCPI_WIFI_IFACE=wlan0
  SOCPI_WIFI_SCAN_INTERVAL=90

  SOCPI_BT_ENABLED=true
  SOCPI_BT_SCAN_INTERVAL=120
"""
...
# Optional Radio Sensors
WIFI_ENABLED = os.getenv("SOCPI_WIFI_ENABLED", "false").lower() == "true"
WIFI_IFACE = os.getenv("SOCPI_WIFI_IFACE", "wlan0")
WIFI_SCAN_INTERVAL = int(os.getenv("SOCPI_WIFI_SCAN_INTERVAL", "90"))

BT_ENABLED = os.getenv("SOCPI_BT_ENABLED", "false").lower() == "true"
BT_SCAN_INTERVAL = int(os.getenv("SOCPI_BT_SCAN_INTERVAL", "120"))
...
wifi_state: Dict[str, Any] = {"ts": "", "aps": []}  # list of APs
bt_state: Dict[str, Any] = {"ts": "", "devices": []}  # list of BT devices
...
<head>
  ...
  <style>
    ...
    header { background:var(--card); padding:15px 30px; border-bottom:2px solid var(--accent); display:flex; justify-content:space-between; align-items:center; box-shadow:0 4px 20px rgba(0,0,0,.5); }
    .scan-status { display:flex; align-items:center; gap:10px; font-family:'JetBrains Mono'; font-size:.8rem; color:var(--accent); }
    ...
    .container { padding:25px; max-width:1700px; margin:auto; }
    .grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(280px,1fr)); gap:20px; margin-bottom:25px; }
    .card { background:var(--card); border-radius:12px; padding:20px; border:1px solid var(--border); transition:.3s; }
    .card:hover { border-color:var(--accent); transform:translateY(-2px); }
    .stat-label { font-size:.75rem; text-transform:uppercase; color:#8892b0; letter-spacing:1px; }
    ...
    th { background:#1e2530; padding:15px; text-align:left; font-size:.75rem; text-transform:uppercase; color:#8892b0; }
    td { padding:12px 15px; border-bottom:1px solid var(--border); font-family:'JetBrains Mono'; font-size:.85rem; vertical-align:top; }
    ...
    .badge { padding:4px 8px; border-radius:4px; font-size:.7rem; font-weight:bold; display:inline-block; }
    .badge-danger { background:var(--danger); color:white; }
    .badge-warn { background:var(--warn); color:black; }
    .badge-success { background:var(--success); color:black; }
    .muted { color:#8892b0; font-family:Inter; font-size:.85rem; }
    .pill { padding:2px 8px; border:1px solid var(--border); border-radius:999px; font-size:.72rem; color:#cbd5e1; display:inline-block; margin-left:6px; }
    ...
    .evt { border:1px solid var(--border); border-radius:10px; padding:10px 12px; margin-bottom:10px; background:#121621; }
    .evt h5 { margin:0 0 6px 0; font-size:.9rem; }
    .evt .meta { font-family:'JetBrains Mono'; font-size:.75rem; color:#94a3b8; }
    .evt .sevCRITICAL { color:var(--danger); }
    .evt .sevMEDIUM { color:var(--warn); }
    .evt .sevINFO { color:var(--success); }
    .topbar { display:flex; gap:12px; align-items:center; }
    .btn { border:1px solid var(--border); background:#1e2530; color:var(--text); padding:8px 10px; border-radius:8px; font-family:'JetBrains Mono'; font-size:.8rem; cursor:pointer; }
    ...
    .modal-content { background:var(--card); margin:4% auto; padding:24px; width:min(900px, 92%); border-radius:15px; border:1px solid var(--accent); box-shadow:0 0 50px rgba(0,242,255,.15); }
    .close { float:right; font-size:22px; cursor:pointer; color:var(--danger); }
    .kv { display:grid; grid-template-columns: 1fr 2fr; gap:8px 14px; font-family:'JetBrains Mono'; font-size:.85rem; }
    ...
    .section-title { margin:0 0 12px 0; font-family:'JetBrains Mono'; font-size:.85rem; color:var(--success); }
  </style>
</head>
<body>
<header>
  <div>
    <h2 style="margin:0;">SOC Pi <span style="font-weight:300; color:var(--accent);">v11.0</span></h2>
    <small id="last-update" style="color:#8892b0;">Booting…</small>
  </div>
  <div class="topbar">
    <a class="btn" href="/api/export.csv" target="_blank">Export CSV</a>
    <div class="scan-status"><div class="pulse"></div> SOC MONITORING</div>
  </div>
</header>
...
<div class="container">
  ...
  <div class="row">
    <div class="card">
      <h4 style="margin-top:0;">Device Trend (Last 50 Scans)</h4>
      <canvas id="trendChart" height="110"></canvas>
    </div>
    <div class="card">
      <h4 style="margin-top:0;">Vendor Distribution</h4>
  ...
  <div class="row" style="margin-top:20px;">
    <div class="table-container">
      <div style="padding: 15px; display: flex; gap: 15px; background: #1e2530;">
        <input type="text" id="search" placeholder="Filter by name, IP, MAC, vendor, OS, type, or port…" style="flex: 2;">
    ...
    <div>
      <div class="card">
        <h4 style="margin-top:0;">Security Events (latest)</h4>
        <div class="events" id="events"></div>
      </div>

      <div class="card" style="margin-top:20px;">
        <h4 style="margin-top:0;">Radio Sensors</h4>
        <div class="muted" id="radio-meta">Wi-Fi/BT: disabled</div>
      ...
      <div class="card" style="margin-top:20px; border-color:#333;">
        ...
        <pre id="log-output" style="font-family:'JetBrains Mono'; font-size:.75rem; color:#aab2c0; max-height:160px; overflow-y:auto; margin:0;"></pre>
      </div>
    </div>
  </div>
</div>
...
async function refreshData() {
  try {
    ...
    const evts = await fetch("/api/alerts?limit=25").then(r => r.json());
    ...
    document.getElementById('last-update').innerText =
      `Last Scan: ${snap.timestamp || "N/A"} | Subnet: ${snap.subnet || "N/A"} | Offline after: ${snap.offline_after || ""}s`;
...
function renderEvents(events){
  ...
  if(!events.length){ box.innerHTML = `<div class="muted">No events yet.</div>`; return; }
  box.innerHTML = events.map(e => {
    const sev = e.severity || "INFO";
    const cls = "sev" + sev;
    return `
      <div class="evt">
        <h5><span class="${cls}">${sev}</span> • ${e.kind || "EVENT"}</h5>
        <div class="meta">${e.ts || ""}</div>
        <div style="margin-top:6px; font-family:JetBrains Mono; font-size:.82rem; color:#cbd5e1;">${e.title || ""}</div>
      </div>
    `;
  }).join("");
}
...
function renderRadio(r){
  ...
  meta.innerText = `Wi-Fi: ${r.wifi?.enabled ? "enabled" : "disabled"} | BT: ${r.bt?.enabled ? "enabled" : "disabled"}`;

  const wifiList = document.getElementById("wifi-list");
  const aps = r.wifi?.aps || [];
  wifiList.innerHTML = aps.length
    ? aps.slice(0,8).map(a => `<div style="font-family:JetBrains Mono; font-size:.8rem; margin-bottom:6px;">${a.ssid || "<hidden>"} <span class="pill">${a.bssid}</span> <span class="pill">${a.signal || ""}</span></div>`).join("")
  ...
  btList.innerHTML = devs.length
    ? devs.slice(0,8).map(d => `<div style="font-family:JetBrains Mono; font-size:.8rem; margin-bottom:6px;">${d.name || "Unknown"} <span class="pill">${d.mac}</span></div>`).join("")
...
def emit_event(kind: str, severity: str, title: str, details: Optional[dict] = None):
    evt = {
        "ts": now_ts(),
        "kind": kind,              # NEW_DEVICE / OFFLINE / PORT_CHANGE / RISK_CHANGE / WIFI_NEW_AP / BT_NEW_DEVICE ...
...
def tail_events(limit: int = 25) -> List[dict]:
    ...
    out = []
    for ln in reversed(lines):  # latest first
        try:
...
def load_aliases() -> Dict[str, dict]:
    """
    device_aliases.json format example:
    {
      "AA:BB:CC:DD:EE:FF": {"name":"Living Room TV","zone":"IoT","owner":"Family","notes":"Samsung"},
      "IP:192.168.0.50": {"name":"No-MAC device","zone":"Guest"}
    }
    """
...
def apply_alias(key: str, d: dict, aliases: Dict[str, dict]) -> dict:
    ...
    # also allow lookup by MAC normalized (common)
    if d.get("mac"):
    ...
    d["owner"] = a.get("owner", "")
    d["notes"] = a.get("notes", "")
    # display name = alias else hostname else IP
...
def build_findings_and_recs(open_ports: List[str], services: List[dict], os_guess: str) -> Tuple[List[str], List[str]]:
    ...
    # Generic service-version hinting
    for s in services:
...
def diff_and_emit_events(key: str, curr: dict, prev: Optional[dict]):
    """
    Emits events for important changes.
    prev is baseline entry (persisted), not inventory.
    """
    if prev is None:
        emit_event(
            kind="NEW_DEVICE",
    ...
    # Vendor change (MAC spoofing edge case)
...
def wifi_scan_iw(iface: str) -> List[dict]:
    ...
    Parses minimal AP list: BSSID, SSID, signal.
    """
    if not have_cmd("iw"):
        return []
    rc, out, err = run_cmd(["iw", "dev", iface, "scan"], timeout=40)
    if rc != 0:
        return []
    ...
    # de-dup by BSSID keep strongest-ish (first)
    seen = set()
    uniq = []
    for a in aps:
        if a["bssid"] in seen:
...
def bt_scan_bluetoothctl() -> List[dict]:
    ...
    # start scan briefly (doesn't always return discovered list, but warms cache)
    run_cmd(["bluetoothctl", "--timeout", "8", "scan", "on"], timeout=15)

    rc, out, err = run_cmd(["bluetoothctl", "devices"], timeout=15)
    if rc != 0:
        return []
...
def background_scanner():
    ...
    while True:
        ...
        try:
            ...
            # mark everyone offline first (we will flip to online if seen)
            with inventory_lock:
                for k, d in device_inventory.items():
                    d["state"] = d.get("state", "ONLINE")
            ...
            for host in discovered:
                ...
                prof = {"mac": disc_mac, "vendor": disc_vendor, "hostname": disc_hostname, "os": "Unknown", "ports": [], "services": []}

                if do_profile:
                    logger.info(f"Profiling {ip} (XML)…")
                    parsed = nmap_profile_xml(ip)
                    prof.update(parsed)
                else:
                    if existing:
                ...
                # hostname enrichment: reverse DNS if missing
                if not prof.get("hostname"):
                    rdns = reverse_dns(ip)
                    if rdns:
                        prof["hostname"] = rdns

                # vendor enrichment if unknown but MAC exists
                ...
                # Online/offline state based on seen time
                state = "ONLINE"
                ...
                # Apply aliases (friendly name/zone/owner)
                ...
                # Diff vs baseline and emit events
            ...
            # OFFLINE detection (based on last_seen_epoch)
            with inventory_lock:
                for k, d in device_inventory.items():
                    ...
                    if (time.time() - last_epoch) >= OFFLINE_AFTER_SEC:
                        if d.get("state") != "OFFLINE":
                            d["state"] = "OFFLINE"
                            # baseline event + persist
                            prev = baseline.get(k, {})
                            curr = {**prev, "state": "OFFLINE", "display_name": d.get("display_name", d.get("ip","Device")), "ip": d.get("ip"), "mac": d.get("mac")}
...
def api_snapshot():
    default = {"timestamp": "", "subnet": NETWORK_CIDR, "offline_after": OFFLINE_AFTER_SEC, "counts": {"seen": 0, "new": 0, "critical": 0}, "devices": []}
